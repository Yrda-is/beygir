...
function sníðaNiðurstöðu(gildi: string | undefined): string {
  return hreinsaTöflugildi((gildi ?? "").replaceAll("uppslög/ít.", "uppslög"));
}

function sníðaValfrjálsarNanósekúndur(ns: number | null): string {
  return ns === null ? "n/a" : sníðaTöflutíma(ns);
}

function sníðaTöflutíma(ns: number): string {
  // Þröngt óskipt bil til að forðast línuskipti.
  return sníðaNanósekúndur(ns).replace(" ", "\u202f");
}
...
function smíðaTöflu(tafla: string, niðurstöður: readonly NiðurstaðaJson[]): string {
  ...
  for (const niðurstaða of niðurstöður) {
    ...
    const röð = [
      ...
      sníðaNiðurstöðu(afköst.niðurstaða),
      sníðaTöflutíma(niðurstaða.mælingar.p50_ns),
...
export async function smíðaAfköst(úr?: string): Promise<string> {
...
if (import.meta.main) {
  const slóð = await smíðaAfköst(þáttaViðföng(Bun.argv.slice(2)));
