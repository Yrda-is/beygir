...
export class Kjarnalesari implements LokanlegurBeygir {
  ...
  finnaBeygingarfærslur<Valið>(
      ...
      fyrirHverjaLeitarniðurstöðu(
        ...
        (stofnsæti, _staðbundiðOrðmyndarsæti, orðmyndasæti) => {
          ...
          if (stofnsæti !== síðastaStofnsæti) {
            ...
            // Þétta `finnaBeygingarfærslur`-leiðin var mæld bæði með þessari endurnýtingu
            // og með beinni `léttFærslaÚrSætum(...)`-leið. Niðurstaðan var að undirbúinn
            // léttur stofn borgar sig skýrt í algengum fjölraða eða fjölstofna tilvikum,
            // þótt hreint einnar raðar tilvik geti stundum orðið örlítið hægara. Sjá
    ...
    // Sama endurnýting var mæld fyrir vörpunarleiðina: að undirbúa ítarlegan
    // stofn einu sinni og nota hann fyrir margar raðir. Það skilaði ekki
    // marktækum ávinningi og var oft örlítið hægara, svo vörpunin smíðar
    // ítarlegu færsluna áfram beint með `ítarlegFærslaBeintÚrSætum(...)`.
    const niðurstöður: Valið[] = [];
    let síðastaStofnsæti = -1;
    let síðastaOrðPassar = true;
    fyrirHverjaLeitarniðurstöðu(
